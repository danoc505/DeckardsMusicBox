...
/* ── AND IT IS A MODULE, because the battery prints it on every run ───────
   THE OWNER: "FIX THE TEST SO THAT IT ALWAYS prints ALL the midi notes for all
   the instruments for seed 1 and a random seed."

   So `score()` is the printer and it takes an already-evaluated MK2. Run this
   file directly and it evaluates the page itself and prints one seed; require
   it from `mk2_test.js` and the battery prints two at the end of every run
   without composing the program twice. One implementation, two callers — the
   alternative was a second copy of the note printer, and two copies of one
   thing is how this file has gone stale four times. */
...
/* ── run directly: evaluate the page and print one seed ──────────────────── */
if(require.main === module){
  ...
  global.document = { getElementById: () => ({ addEventListener(){}, textContent: "", value: "1", innerHTML: "" }),
                      createElement: () => ({ click(){} }) };
  eval(src);
  const lines = score(global.window.MK2, {
    genre: argOf("--genre", null) || undefined,
    seed:  parseInt(argOf("--seed", "1"), 10),
    from:  parseInt(argOf("--from", "0"), 10),
    to:    parseInt(argOf("--to", "999999"), 10),
  });
