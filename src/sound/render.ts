...
import {
  DEFAULTS, DRUM_LANES, PEDALS_ADD, PEDAL_ORDER, ROLES, SENDS,
...
import { flute, horns, organ, pad, pluck, rhodes, sub, wurly, type NoteIn } from "./voices.ts";
...
class Channel {
  ...
  // this part's own pedal board, held across knob moves
  ...
  run(n: number, t: number, S: SoundRules, sr: number): void {
    ...
    let input = src;
    const rig = this.pedals;
    if (ch.pedals > 0 && rig !== null && !rig.empty) {
      const out = this.boarded;
      for (let i = 0; i < n; i++) out[i] = src[i]! * (1 - ch.pedals) + rig.run(src[i]!) * ch.pedals;
      input = out;
    }
...
export class Engine {
  ...
  constructor(song: Song, opts: RenderOptions = {}) {
    ...
    this.voices = { rhodes, wurly, sub, pluck, organ, pad, flute, horns };
