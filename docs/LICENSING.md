...
| **BBC Sound Effects archive** (MK2's SCENE bank, 2026-08-20) | trimmed, measured, DC-corrected ADPCM excerpts of 8 recordings — a bullfrog chorus, two cricket/night beds, a great northern diver, a tawny owl, two rivers and a thunderclap; 12–30 s each out of recordings running 64–451 s | BBC RemArc licence, same position as the row above: **personal/educational/research, no commercial use**, weaker than CC0 and stated as such. Archive IDs, the measured window for each, the band it was scored in, and the recordist where the archive names one are all in `harness/scene_bank.py`. Cleaner sources were tried first per the policy above and none was reachable: Wikimedia Commons returned 429 to every request from this host, xeno-canto's API now requires an account key, and archive.org's CC-licensed audio for these terms is podcasts rather than field recordings |
