...
| **`keys2` plays in 72.7% of bars and NO `form.roles` entry asks for it** | The genre's arrangement table never lists `keys2` in any section. It is added by the code to any section that already has something pitched, so the second keyboard is on three bars in four **by default rather than by a decision the genre made**. Together with `keys` (100% of bars) it means Moore's harmonic-filler layer — "fills the middle range between bass and treble" [corpus:mtosmt Schwitzgebel] — is doubled almost always, with `counter` (36.5%) sometimes a third. | Make the second keyboard something a section ASKS for, the way every other role is. This is the cheapest of the three rows here and the only one where the current behaviour is unowned rather than chosen. |
...
| **The deep low-pass is unverified** | The sources' single most specific FX instruction is a low-pass "pulled down to around **2 kHz**" [corpus:modeaudio], corroborated in kind by two others. What lofi's chain actually does to the top end has not been measured. | A measurement first — render and look at the spectrum — before any number moves. |
