...
| what | why it was done | what says it worked |
|---|---|---|
| **A board is tuned, not rebuilt — a pedal can be automated** (`pedals.ts`, `dsp.ts`, `render.ts`) | `pedals.ts` had no `set` on anything, so the only way to change a pedal's number was to build a new pedal, and `Channel.tune` did that whenever any number on the board differed. `retune()` runs every 1024 samples while the desk moves, so automating one knob rebuilt the board 21 times a second and every pedal on it lost its state | The tremolo beside an automated knob kept **0.0161** of its wobble against **0.0894** standing still; it keeps **0.0972** now. Records byte-identical except where a board treatment lands — lofi 42 and both dungeon synth seeds identical, lofi 2 differs by −42.9 dB because it fires `waver`. `rack.test.ts` holds it and fails on the old rebuild |
| **~~A genre states a pedal cycle~~ — BUILT, MEASURED, DELETED** | soundonsound, quoted in `EFFECTS-IN-TIME.md` §1B: "draw in a tremolo that gets steadily deeper and faster as each chord decays". lofi's lead has that tremolo, so the move was written as a one-bar ramp on its depth and rate | It moves the LEAD −21 to −24 dB and **the record −34.6 to −39.9 dB**, against `brighten` REFUSED on lofi at −37.7. Depth is not the lever: 0.5 → 1.0 buys 2.4 dB. Cause is `TALLY.md` §2's own finding — one part on the board at 0.35 feed. Dungeon synth's board would carry it (−27.9 to −29.5 dB) but the only source for the gesture is already spent on `rack.pole.hz`, and a second gradual brightening is a knob that does what the knob beside it does. The mechanism ships; the cycle does not |
| **Dungeon synth's boards say who is standing on them** (`dungeonsynth.ts`) | Two of its own comments were false about it and had been for as long as they had been written: the divider says "the pad must not clock it" and the pad clocked it at 0.7 of the feed; the mix calls the flute "the one voice in the room that is not coming out of an amp" and ran it through a Big Muff at 0.15 | Divider off the pad, board off the flute, feed with it; nothing else moved. **Not one note changed** — `measure.ts --sweep dungeonsynth 1 20` byte-identical on `--map` and `--parts`, vocabulary 21 before and after. The record moved **−20.7 / −19.4 / −19.4 dB** on seeds 2, 42, 7, which sits between `push` and `widen` on this genre's own table. The flute alone moves −20.7 to −23.1 dB and comes back 0.6 dB louder; the pad alone −11.3 to −12.7 dB |
| **A pedal board per part** (`spec.ts`, `render.ts`, `reach.ts`, `treat.ts`, both genres, the page) | One `pedals: PedalsSpec` sat under the whole band and `board(S.pedals, sr)` built every part's chain out of it. No document states that and the README said the opposite, so it was an accident, and it had already made two of dungeon synth's own comments false — "the pad must not clock it" while the pad clocked the divider at 0.7, and a flute "not coming out of an amp" running through a Big Muff | Six records over both genres **byte-identical** after the migration, which is the proof the mechanism change is only a mechanism change. `boardWalked` now asks "is this part fed" and "is this part's board lit" of the same part, which it could not before. The page gives each pedal a bank of switches, one per part, and a driven page keeps two parts' settings for the same Muff apart |
...
| **The character's job is drawn once** (`assign()` in `arrange.ts`) | An ostinato "persistently repeats in the same musical voice"; a counter that arpeggiates idea A and plays a line in idea B is two characters wearing one name | lofi's character changed job between materials **30% → 0%**; dungeon synth unmoved |
| **The character is never hushed** | The yield around a protagonist is one-directional (Johnston) | Hushes aimed at the character **39% → 0%** (lofi), **41% → 0%** (dungeon synth) |
| **Newcomers walk in one per boundary** | Max Martin, Johnston, EDMProd's "drop-off"; seed 42 went 2 → 6 parts at bar 8 | Openings with 2+ arriving **28% → 25%** (lofi); the residue is the peak, which has everyone, and the per-round parts that cannot walk in. arrange 24/24 |
| **`COST.mask`: the keys pay to double inside the bass's band or the character's** (`keys.ts`) | Owsinski's second rule; the diagnosis's "keys block on the bass". Doublings only: whole voices slid the hand into the tune's band (50 → 56% above the lead's floor) | lofi keys in the bass band **16% → 12%**, tune on top **80% → 82%**, hand 2.34 → 2.25 voices/strike. Dungeon synth unmoved. Swept 0/4/6/8/12 |
